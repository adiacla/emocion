...
#python -m venv env
#cd env\Scripts
#cd C:\Users\Usuario\Documents\Proyecto IA\emociones\Face_Emotion-master
#cd C:\Users\Usuario\Documents\Proyecto IA\smart_gpt
#python.exe -m pip install --upgrade pip

#pip install tensorflow==2.15.0
#python -c 'import keras; print(keras.__version__)'
#python -c 'imtensorflow as tf; print(tf.__version__)'
#pip install imutils
...
classes = ['enojado', 'asco', 'miedo', 'feliz', 'neutral', 'triste', 'sorpresa']
...
emotionModel = load_model("modelFEC2.h5")
...
while True:
	...
	cv2.putText(frame, str(int(fps))+" FPS", (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 0, 100), 3, cv2.LINE_AA)
	cv2.putText(frame, "Smart Region Lab", (8, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 100), 2, cv2.LINE_AA)
	cv2.putText(frame, "Cual es tu expresion", (10, 105), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 100), 3, cv2.LINE_AA)
	logo_img = cv2.imread("logo/logo.png") 
	logo_img_resized = cv2.resize(logo_img, (80, 80))
	y_offset = 20
	x_offset = frame.shape[1] - logo_img_resized.shape[1] - 20
	frame[y_offset:y_offset+logo_img_resized.shape[0], x_offset:x_offset+logo_img_resized.shape[1]] = logo_img_resized
